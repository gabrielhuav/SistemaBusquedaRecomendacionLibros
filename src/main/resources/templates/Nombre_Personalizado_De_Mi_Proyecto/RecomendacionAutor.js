...
function recuperarImagen(idAutor) {
    ...
    .then(data => {
        if (data.photos && data.photos.length > 0) {
            ...
            console.log('No se encontraron fotos para este autor');
            document.getElementById("img").src = 'default_image.jpg'; // Imagen por defecto
        }
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.docs.length > 0) {
                ...
                if (libro.editions && libro.editions.docs.length > 0) {
                    ...
                    console.log('No se encontraron ediciones para este libro');
                }
            } else {
...
function mostrarHistorial() {
    let idUsuario = document.getElementById("idUsuario").value;
    const url = '/AutorFavorito/historial/' + idUsuario;

    fetch(url)
       .then(response => response.json())
       .then(data => {
            let tabla = document.getElementById("tabla-historial");
            console.log(tabla); // Add this line to check if tabla is null
            if (tabla === null) {
                console.error("Tabla element not found!");
                return;
            }
            tabla.innerHTML = `
                <tr>
            ...
            data.forEach(function(dato) {
                ...
                fila.innerHTML = `
                    <td><img src="https://covers.openlibrary.org/b/id/${dato.imagen}-M.jpg" class="box"></td>
                    <td>${dato.titulo}</td>
